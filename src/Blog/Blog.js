...
function OpenGraph() {
    return (
        <Helmet>
            <title>Blog | Kris Kopczynski Portfolio </title>
            <link
                rel='canonical'
                href='https://k2project.github.io/portfolio/blog/'
            />
            <meta
                name='description'
                content='Bits and pieces on my journey to become a better web developer.'
            />

            <meta
                property='og:title'
                content='Kris Kopczynski Portfolio | Blog'
            />
            <meta
                property='og:url'
                content='https://k2project.github.io/portfolio/blog/'
            />
    ...
    return (
        <div className='BlogPage__noPosts'>
            <div className='wrapper'>
                <p className='p_large'>
                    {' '}
                    I have no posts yet, but I'm working on it. Please check
                    with me again later.
                </p>
            </div>
        </div>
...
function FeaturedPostList() {
    ...
    const postFreaturedDisplay = arr.map(post => (
        <BlogPostLink post={post} key={'featuredPosts_' + post.id} />
    ));
    function showAll(e) {
        e.target.remove();
        setArr(postFeatured);
    ...
    return (
        <div className='BlogPostsList__featured' id='posts-featured'>
            ...
            <div className='wrapper'>{postFreaturedDisplay}</div>
            {postFeatured.length > 6 && (
                <div className='txt_right'>
                    <button class='Blog__btn' onClick={showAll}>
                        Show all
                    </button>
                </div>
            )}
...
function RecentPostList() {
    ...
    const pages =
        posts.length % n ? Math.floor(posts.length / n) + 1 : posts.length / n;
    const pagesBtns = new Array(pages).fill(posts.length).map((p, index, a) => (
        <button key={'pag_recentPosts_' + index} data-page={index}>
            <span className='sr-only'>
                Posts from {index * n + 1} to{' '}
                {index === a.length - 1 ? p : index * n + n}
            </span>
            {++index}
        </button>
    ));
    function getPage(e) {
        ...
        if (btn) {
            ...
            const rangeEnd =
                index === pagesBtns.length - 1 ? posts.length : index * n + n;
            let nums =
                rangeStart === rangeEnd
                    ? rangeEnd
                    : `${rangeStart} - ${rangeEnd}`;
            ...
            setTimeout(() => {
                document
                    .querySelector('.BlogPostsList__recent')
                    .scrollIntoView();
            }, 100);
            document
                .querySelectorAll('.Blog__pagination button')
    ...
    const postRecentDisplay = arr.map(post => (
        <BlogPostLink post={post} key={'recentPosts_' + post.id} />
    ));
    return (
        <div className='BlogPostsList__recent'>
            <div className='Blog__divider'> Most Recent Posts </div>
            {posts.length > 5 && (
                <div className='Blog__range'>
                    {range} / {posts.length}
                </div>
            )}
            <div className='wrapper'>{postRecentDisplay}</div>
            {posts.length > 5 && (
                <div className='Blog__pagination' onClick={getPage}>
                    {pagesBtns}
                </div>
            )}
        </div>
...
function BlogPostsLiked() {
    ...
    const postLikedDisplay = arr.map(post => (
        <BlogPostLink post={post} key={'likedPosts_' + post.id} />
    ));
    return (
        <div className='BlogPostsList__liked' id='posts-liked'>
            <div className='Blog__divider'> Posts You Saved</div>
            <div className='wrapper'>{postLikedDisplay}</div>
            {postsLiked.length > 6 && (
                <div className='txt_right'>
                    <button class='Blog__btn' onClick={showAll}>
                        Show all
...
function BlogPostLink(props) {
    ...
    return (
        <NavLink
            to={getPostLink(props.post)}
            key={'props.post_' + id}
            className='BlogPostLink'
        >
            <div
                className='BlogPostLink__img'
                style={{ backgroundImage: 'url(' + bg + ')' }}
            >
                <Stickers
                    sm
                    featured={featured ? true : null}
                    liked={likes.arr.includes(+id) ? true : null}
                />
            </div>
            <div className='BlogPostLink__desc'>
                <p className='BlogPostLink__title'>
                    <b>{title}</b>
                </p>
                {subtitle && (
                    <p className='BlogPostLink__subtitle'>{subtitle}</p>
                )}
...
function BlogPostSelected(props) {
    const displaySelectedPosts = props.postsSelected.map(post => (
        <BlogPostLink post={post} key={'selectedPosts_' + post.id} />
    ));
    return (
        <div className='BlogPostsList__selected '>
            <div className='wrapper'>{displaySelectedPosts}</div>
        </div>
    );
}
